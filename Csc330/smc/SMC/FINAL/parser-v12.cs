...
class Parser {
	...
	List<Function> DeclaredFunctionList;
	List<Function> UsedFunctions;
	
	private class Function {
		public string FnName { get; set; }
		public int FnArgs { get; set; }
		public Function(string fnName, int fnArgs) {
			FnName = fnName;
			FnArgs = fnArgs;
		}
	}
    ...
    public int DoParse() {
        ...
        try {
			...
			/*
			 * Had to change this to use a SemanticError since the trivial example
			 * func Main() { 
			 * 		return 0;
			 * }
			 * func Main() {
			 * 		return 0;
			 * }
			 * Would continue running even though a duplicate method was encountered.
			 */
            SemanticError("{0}", e.Message);
        }
		checkFunctionCalls();
	...
	{
		foreach (Function UsedFun in UsedFunctions)
		{
			bool isDefined = false;
			foreach (Function DeclaredFun in DeclaredFunctionList)
			{
				if (UsedFun.FnName == DeclaredFun.FnName)
					isDefined = true;
				if (DeclaredFun.FnName == UsedFun.FnName &&
					DeclaredFun.FnArgs != UsedFun.FnArgs)
					SemanticError("Invalid number of arguments in function call : {0}", UsedFun.FnName);
			}
			if (!isDefined && UsedFun.FnName != "Read" && UsedFun.FnName != "Write" && UsedFun.FnName != "WriteLine")
				SemanticError("Undefined function call : {0}", UsedFun.FnName);
		}
	}
	...
	{
		foreach (Function f in DeclaredFunctionList)
		{
			if (f.FnName == fnName)
    ...
    void matchFunction() {
		...
		DeclaredFunctionList.Add(new Function(fnName, fnArgsCount));
        currFunction = null;
    ...
    void matchAssignmentOrCall() {
        ...
        if (nextToken == Token.LPar) {
			...
			UsedFunctions.Add(new Parser.Function(target, argCnt));
    ...
    void matchFactor() {
        ...
        case Token.Ident:
            ...
            if (nextToken == Token.LPar) {
				...
				UsedFunctions.Add(new Function(id, argcnt));
            } else {
