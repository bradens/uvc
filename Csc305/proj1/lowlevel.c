...
//	res->red = (sP->v_color.red * x) >> 8 + (eP->v_color.red * (256-x)) >> 8;
//	res->green = (sP->v_color.green * x) >> 8 + (eP->v_color.green * (256-x)) >> 8;
//	res->blue = (sP->v_color.blue * x) >> 8 + (eP->v_color.blue * (256-x)) >> 8;
	int bx = sP->x;
	int by = eP->v_color.red;
	int dx = eP->x - sP->x;
	if (dx < 0) dx = (-1*dx);
	int dy = eP->v_color.red - sP->v_color.red;
	if (dy < 0) dy = (-1*dy);
	int p = 2 * (dy - dx);
	int const2 = 2 * (dy - dx);
	int const1 = 2 * dy;
...
void drawLeftTriangle(vertex* b, vertex* t, vertex* m) {
  ...
  color bColor = b->v_color;
  color tColor = t->v_color;
  color mColor = m->v_color;

  printf("Bottom Vertex color: (%d, %d, %d)\n", bColor.red, bColor.green, bColor.blue);
  printf("Top Vertex Color: (%d, %d, %d)\n", tColor.red, tColor.green, tColor.blue);
  printf("M Vertex Color: (%d, %d, %d)\n", mColor.red, mColor.green, mColor.blue);
