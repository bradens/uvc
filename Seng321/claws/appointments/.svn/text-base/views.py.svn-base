...
def log(request, event_id):
	...
	if inEmpGroup(request.user) or inManGroup(request.user):
		...
		return render_to_response('appointments/log.html', {'app': app, 'back': request.META.get('HTTP_REFERER', '')})
	else:
		return HttpResponseForbidden()
	
@login_required
@user_passes_test(lambda u: inEmpGroup(u) or inManGroup(u), login_url='/')
def updateLog(request, event_id):
	if request.method == 'POST':
		...
		if len(logText) > 0:
			...
			app.log = logText
			app.save()
	return HttpResponseRedirect("/appointments/" + event_id)
...
def cam(request, event_id):
	...
	if inEmpGroup(request.user) or inManGroup(request.user) or app.customer_id == request.user:
		if app.camera_id == -1:
			...
			return render_to_response('appointments/cam.html', {'app': app, 'gifnum': app.camera_id, 'back': request.META.get('HTTP_REFERER', '')})
	else:
		return HttpResponseNotFound()
	...
	if request.method == 'POST':
		try:
			...
			try:
				...
				return HttpResponseNotFound()
		if not (inEmpGroup(request.user) or inManGroup(request.user)):
			if not app.customer_id == request.user:
				return HttpResponseForbidden()
		app.cat_name = request.POST.get('cat_name', '')
		app.cat_breed = request.POST.get('cat_breed', '')
		app.special_considerations = request.POST.get('special_considerations', '')
		app.start = request.POST.get('start_time', '')
		app.end = request.POST.get('end', '')
		app.camera_id = request.POST.get('camera_id', '')
		if (inEmpGroup(request.user) or inManGroup(request.user)) and not request.POST.get('user', '') == app.customer_id.id:
			app.customer_id = User.objects.filter(id=request.POST.get('user', ''))[0]
		app.save()
		return HttpResponseRedirect('/appointments/' + event_id)
	return HttpResponseForbidden();
