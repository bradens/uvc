...
var Timeframe = Class.create({
  ...
  initialize: function(element, options) {
    ...
    this.original_range = $H({});
    this.range = $H({});
    this.earliest = Date.parseToObject(this.options.get('earliest'));
    this.latest   = Date.parseToObject(this.options.get('latest'));
    this.range.set("start", Date.parseToObject(this.options.get('range_start')));
    this.range.set("end", Date.parseToObject(this.options.get('range_end')));
    this.original_range.set("start", Date.parseToObject(this.options.get('range_start')));
    this.original_range.set("end", Date.parseToObject(this.options.get('range_end')));
  ...
  parseUnavailable: function(ud)
  {
		// parse unavailable days
		this.unavailableDays = new Array();
		dates_arr = ud.split(",");
		for (i = 0;i < dates_arr.length;i++)
		{
			d_arr = dates_arr[i].split("-");
			nd = new Date(d_arr[0], d_arr[1]-1, d_arr[2]);
			if (nd != null && this.original_range.get("start") != null && this.original_range.get("end") != null)
			{
				if (!(nd.getFullYear() >= this.original_range.get("start").getFullYear() && 
					nd.getMonth() >= this.original_range.get("start").getMonth() && 
					nd.getDate() >= this.original_range.get("start").getDate() &&
					nd.getFullYear() <= this.original_range.get("end").getFullYear() && 
					nd.getMonth() <= this.original_range.get("end").getMonth() && 
					nd.getDate() <= this.original_range.get("end").getDate()))
					this.unavailableDays.push(nd);
			}
			else if(nd != null && (this.original_range.get("start") == null || this.original_range.get("end") == null))
				this.unavailableDays.push(nd);
		}
		this.range = this.original_range;
  ...
  populate: function() {
    ...
    this.calendars.each(function(calendar) {
      ...
      calendar.select('td').each(function(day) {
        ...
        else if (this.unavailable(day.date))
  ...
  unavailable: function(date) {
	if (this.unavailableDays != undefined)
	{
		for (i = 0;i < this.unavailableDays.length;i++)
		{
			if (date.getDate() == this.unavailableDays[i].getDate() && date.getMonth() == this.unavailableDays[i].getMonth() && date.getFullYear() == this.unavailableDays[i].getFullYear())
				return true;
		}
		return false;
	}
  ...
  refreshField: function(fieldName) {
    ...
    if (this.range.get(fieldName)) {
      field.setValue(this.range.get(fieldName).strftime("%Y-%m-%d"));
    } else
      field.setValue('');
  ...
  handleButtonClick: function(event, element) {
    ...
    if (element.hasClassName('next')) {
      if (!this.buttons.get('next').get('element').hasClassName('disabled'))
	  {
		...
		var jqhs = jQuery.get("/appointments/data/getKennellingAv2", { startDay : this.date.strftime("%Y-%m-%d") })
  ...
  extendRange: function(date) {
    ...
    if (date > this.startdrag) {
      ...
      end = this.availableRangeS(this.startdrag, date);
    } else if (date < this.startdrag) {
      start = this.availableRangeE(date, this.startdrag);
      end = this.startdrag;
    } else
      start = end = date;
  ...
  availableRangeS: function(start, end)
  {
	date = start;
	while(date < end)
	{
  ...
  availableRangeE: function(start, end)
  {
	date = end;
	while(date > start)
	{
  ...
  validateRange: function(start, end) {
	...
	if (start == this.startdrag)
	{
		...
		this.range.set('end', this.availableRangeS(start, end));
	}
	else
	{
		this.range.set('start', this.availableRangeE(start, end));
